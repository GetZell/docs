...
<Note>Intensity presets are locked—you select a preset and cannot customize individual values. This ensures consistent training standards across your organization.</Note>
...
<Steps>
  ...
  <Step title="Add Steps to Each Module">
    ...
    **Agent Call Step:** Practice sessions with AI agents
    ...
    <Info>The number of calls required is automatically set by your course's intensity preset. For example, a **Standard** course requires 5 calls per agent step.</Info>
...
Agent call steps require learners to complete practice conversations with AI bots. The number of calls and the minimum score per call are determined by the course's intensity preset.
...
**Pass Criteria:**
- Each call must score at or above the preset's minimum (≥6/10, ≥7/10, or ≥8/10) to count toward completion
...
<Note>Changes to intensity preset affect the call requirements and minimum score per call for all steps going forward. User enrollment is managed directly on course cards.</Note>
